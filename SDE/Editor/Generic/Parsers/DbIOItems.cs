    ...
    public sealed class DbIOItems
    {
        ...
        public static void Loader<TKey>(DbDebugItem<TKey> debug, AbstractDb<TKey> db)
        {
            ...
            var eqpFlags = new Dictionary<string, long>
            {
                ...
                ["acc_rl"] = 0x000008 | 0x000080,
                ["shadow_acc_rl"] = 0x100000 | 0x200000,
                ["both_hand"] = 0x000002 | 0x000020,
                ["right_hand"] = 0x000002,
                ["left_hand"] = 0x000020,
                ["right_accessory"] = 0x000080,
                ["left_accessory"] = 0x000008,
                ["both_accessory"] = 0x000080 | 0x000008,
            ...
            var classFlags = new Dictionary<string, long>
            {
                ...
                ["third_upper"] = 0x10, // 16
                ["third_baby"] = 0x20,  // 32
                ["all_upper"] = 0x02 | 0x10, // 18
                ["all_baby"] = 0x04 | 0x20,  // 34
                ["all_third"] = 0x08 | 0x10 | 0x20, // 56
                ["fourth"] = 0x40 //64
            };
            var dropEffectFlags = new Dictionary<string, int>
            {
                ["none"] = (int)DropEffectType.None,
                ["client"] = (int)DropEffectType.Client,
                ["white_pillar"] = (int)DropEffectType.White_Pillar,
                ["blue_pillar"] = (int)DropEffectType.Blue_Pillar,
                ["yellow_pillar"] = (int)DropEffectType.Yellow_Pillar,
                ["purple_pillar"] = (int)DropEffectType.Purple_Pillar,
                ["orange_pillar"] = (int)DropEffectType.Orange_Pillar,
                ["green_pillar"] = (int)DropEffectType.Green_Pillar,
                ["red_pillar"] = (int)DropEffectType.Red_Pillar
            };

            if (debug.FileType == FileType.Txt) //구형 text db 대응
            {
            ...
            else if (debug.FileType == FileType.Yaml) // yaml RA대응
            {
                ...
                foreach (var item in ele.Output["copy_paste"] ?? ele.Output["Body"])
                {
                    try
                    {
                        ...
                        table.SetRaw(itemId, ServerItemAttributes.Upper, DbIOUtils.LoadFlag(item["Classes"], classFlags, "127"));
            ...
            else if (debug.FileType == FileType.Conf && db.ProjectDatabase.IsNova)
            {
                ...
                foreach (var item in ele.Output["copy_paste"] ?? ele.Output["item_db"])
                {
                    try
                    {
                        ...
                        table.SetRaw(itemId, ServerItemAttributes.Upper, DbIOUtils.LoadFlag(item["Classes"], classFlags, "127", false));
        ...
        public static void WriteEntryYaml(StringBuilder builder, ReadableTuple<int> tuple, MetaTable<int> itemDb)
        {
            if (tuple != null)
            {
                ...
                if ((value = tuple.GetValue<int>(ServerItemAttributes.SubType)) != 0)
                {
                    if (type == 6)
                    {
                        builder.AppendLine("    SubType: " + Constants.ToString<CardType>(value));
                    }
                    if (type == 10)
                    {
                ...
                if ((value = tuple.GetValue<int>(ServerItemAttributes.Upper)) != 127)
                {
        ...
        public static void WriteEntry2(BaseDb db, StringBuilder builder, ReadableTuple<int> tuple, MetaTable<int> clientDb)
        {
            ...
            try
            {
                ...
                if ((value = tuple.GetValue<int>(ServerItemAttributes.Upper)) != 127 && (TypeType)type != TypeType.EtcItem && (TypeType)type != TypeType.Card && (TypeType)type != TypeType.PetEgg)
                {
                    builder.AppendLine("	Classes: {");
