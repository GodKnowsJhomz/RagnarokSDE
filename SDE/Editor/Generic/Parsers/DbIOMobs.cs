    ...
    public sealed class DbIOMobs
    {
            ...
            else if (debug.FileType == FileType.Yaml)
            {
                try
                {
                    ...
                    foreach (var parser in ele.Output["copy_paste"] ?? ele.Output["Body"])
                    {
                        ...
                        // 추가
                        table.SetRaw(itemId, ServerMobAttributes.Resistance, parser["Resistance"] ?? "0");
                        table.SetRaw(itemId, ServerMobAttributes.MagicResistance, parser["MagicResistance"] ?? "0");
                        ...
                        // 추가 
                        table.SetRaw(itemId, ServerMobAttributes.GroupId, parser["GroupId"] ?? "0");
                        table.SetRaw(itemId, ServerMobAttributes.Title, parser["Title"] ?? "");
                        table.SetRaw(itemId, ServerMobAttributes.Ai, parser["Ai"] ?? "");

                        long mode = 0;

                        if (parser["Ai"] != null)
                        {
                            string aiValue = parser["Ai"].ObjectValue;

                            if (Int32.TryParse(aiValue, out int ai)) 
                            {
                                switch (ai)
                                {
                                    ...
                                    default:
                                        throw new Exception("Unknown mob Ai value: " + aiValue);
                                }
                            }
                            else 
                            {
                                switch (aiValue.Trim().Replace("-", "_").Replace(" ", "_").ToUpperInvariant())
                                {
                                    case "ABR_PASSIVE":
                                        mode |= 0x21;
                                        break;
        ...
        public static void WriteEntryYaml(StringBuilder builder, ReadableTuple<int> tuple, MetaTable<int> itemDb)
        {
            if (tuple != null)
            {
                ...
                if ((value = tuple.GetValue<int>(ServerMobAttributes.Resistance)) != 0)
                {
                    builder.AppendLine("    Resistance: " + value);
                }

                if ((value = tuple.GetValue<int>(ServerMobAttributes.MagicResistance)) != 0)
                {
                    builder.AppendLine("    MagicResistance: " + value);
                }
                ...
                if ((value = tuple.GetValue<int>(ServerMobAttributes.GroupId)) != 0)
                {
                    builder.AppendLine("    GroupId: " + value);
                }

                if (!string.IsNullOrEmpty(valueS = tuple.GetValue<string>(ServerMobAttributes.Title)))
                {
                ...
                valueS = tuple.GetValue<string>(ServerMobAttributes.Ai);

                if (valueS != null && valueS != "")
                {
                    builder.AppendLine("    Ai: " + valueS);

                    if (Int32.TryParse(valueS, out value))
                    {
                        switch (value)
                        {
                            case 1: mode &= ~0x81; break;
                    ...
                    {
                        switch (valueS.Trim().Replace("-", "_").Replace(" ", "_").ToUpperInvariant())
                        {
                            case "ABR_PASSIVE":
                                mode &= ~0x21;
                                break;
