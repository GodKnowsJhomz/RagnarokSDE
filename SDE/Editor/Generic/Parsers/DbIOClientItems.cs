    ...
    public sealed class DbIOClientItems
    {
        ...
        public static void WriteEntry(StringBuilder builder, ReadableTuple<int> tuple, bool end = false)
        {
            ...
            if (SdeAppConfiguration.DbWriterItemInfoClassNum)
            {
                ...
                if (SdeAppConfiguration.DbWriterItemInfoIsCostume || !String.IsNullOrEmpty(tuple.GetValue<string>(ClientItemAttributes.EffectID.Index)) || !String.IsNullOrEmpty(tuple.GetValue<string>(ClientItemAttributes.PackageID.Index)))
                {
                    builder.AppendLine(",");
                }
                else
            ...
            if (SdeAppConfiguration.DbWriterItemInfoIsCostume)
            {
                ...
                builder.Append(tuple.GetValue<bool>(ClientItemAttributes.IsCostume.Index).ToString().ToLowerInvariant());

                if (!String.IsNullOrEmpty(tuple.GetValue<string>(ClientItemAttributes.EffectID.Index)) || !String.IsNullOrEmpty(tuple.GetValue<string>(ClientItemAttributes.PackageID.Index)))
                {
                    builder.AppendLine(",");
                }
                else
            ...
            string effectId = tuple.GetValue<string>(ClientItemAttributes.EffectID.Index);
            if (!String.IsNullOrEmpty(effectId))
            {
                builder.Append("\t\tEffectID = ");
                builder.Append(_toAnsiEscaped(effectId));

                if (!String.IsNullOrEmpty(tuple.GetValue<string>(ClientItemAttributes.PackageID.Index)))
                {
                    builder.AppendLine(",");
                }
                else
            ...
            string packageId = tuple.GetValue<string>(ClientItemAttributes.PackageID.Index);
            if (!String.IsNullOrEmpty(packageId))
            {
                builder.Append("\t\tPackageID = ");
                builder.AppendLine(_toAnsiEscaped(packageId));
            }
                ...
                foreach (LuaKeyValue itemProperty in itemProperties.Variables)
                {
                    switch (itemProperty.Key)
                    {
                        ...
                        case "ClassNum":
                            ...
                            break;
                        case "EffectID":
                            table.SetRaw(itemIndex, ClientItemAttributes.EffectID, ((LuaValue)itemProperty.Value).Value);
                            break;
                        case "PackageID":
                            table.SetRaw(itemIndex, ClientItemAttributes.PackageID, ((LuaValue)itemProperty.Value).Value);
                            break;
                        case "costume":
